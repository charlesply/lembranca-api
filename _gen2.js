...
const API='https://suno-api-novo.bvph.uk';  // backend REAL (lembranca-api), não o suno-api-lite morto
const H={apikey:KEY,Authorization:'Bearer '+KEY};
const ORDER='49dcdd8d-3f54-41e9-a6fa-e5f5a26c8463';
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
(async()=>{
 // health
 try{const h=await axios.get(`${API}/api/test-client`,{timeout:15000});console.log('health:',h.status,JSON.stringify(h.data).slice(0,120));}
 catch(e){console.log('health FAIL:',e.response?.status||e.code,String(e.response?.data||e.message).slice(0,80));return;}
 ...
 console.log('gerando:',o.honoree_name,'| tags:',tags,'| SEM phone (sem notificação)');
 ...
 for(let i=0;i<36;i++){
   ...
   process.stdout.write(`[${i}] ${c.status} full=${n} `);
   if(n>=1||c.original_audio_url){console.log('\n✅ AUDIO PRONTO\nfull:',JSON.stringify(c.full_audio_urls));break;}
 }
})().catch(e=>console.error('ERR',e.response?.status,e.message,JSON.stringify(e.response?.data||'').slice(0,150)));
