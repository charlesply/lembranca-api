...
// ⚠️ Devem casar EXATAMENTE os templates aprovados no ComprarSMS (parte fixa idêntica):
//   "Lembrança Cantada - Previa" (recuperação) e "Lembrança Cantada - Pago" (entrega).
const TPL_RECOVERY = process.env.SMS_TPL_RECOVERY
  || 'Ola {nome}! Sua musica ficou pronta. Ouca a previa e libere a completa: {link}';
...
async function sendSmsBatch(items) {
  ...
  if (!list.length) return { skipped: 'empty_after_norm' };
  // 🚨 SEMPRE 1 request POR pessoa. NUNCA agrupar mensagens diferentes num único
  // recipients[]: o ComprarSMS aplicava a msg/link do 1º recipient a TODOS os
  // números do lote (incidente 28/ago: 40 pessoas receberam o link de 1 só).
  try {
    const token = await getToken();
    let sent = 0, fail = 0, last = null;
    for (const it of list) {
      try {
        const r = await axios.post(`${SMS_BASE}/api-sms-send`,
          { phone_numbers: [it.phone], message: it.message },
          { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, timeout: 20000 });
        last = r.data;
        if (r.data?.status === 'completed' || Number(r.data?.success_count || 0) > 0) sent++;
        else { fail++; console.warn('[sms] ⚠️', JSON.stringify(r.data).slice(0, 200)); }
      } catch (e1) {
        fail++;
        console.error('[sms] ❌ item:', e1.response?.data ? JSON.stringify(e1.response.data).slice(0, 200) : e1.message);
      }
    }
    console.log(`[sms] enviados ${sent}/${list.length} (falhas ${fail})`);
    return { ok: sent > 0 && fail === 0, sent, fail, response: last };
  } catch (e) {
