...
// ── Alto nível: recuperação (não pagou) ──────────────────────────────────────
// order: { id, customer_name, phone }
async function sendRecoverySms(order) {
  const to = formatPhone(order?.phone);
  if (!to || !order?.id) return { skipped: 'sem_to_ou_id' };
  const { msg, form, linkUrl } = buildMsg({
    name: order.customer_name,
    textNoLink: 'sua musica ficou pronta. Ouca a previa e libere a completa:',
    linkUrl: `${SITE}/finalizar/${order.id}?src=sms`,
  });
  return sendSmsBatch([{ to, tipoEnvio: '2', msg, id: `rec-${order.id}`.slice(0, 50), form, linkUrl }]);
}

// ── Alto nível: entrega (pagou) ──────────────────────────────────────────────
async function sendDeliverySms(order) {
  const to = formatPhone(order?.phone);
  if (!to || !order?.id) return { skipped: 'sem_to_ou_id' };
  const { msg, form, linkUrl } = buildMsg({
    name: order.customer_name,
    textNoLink: 'sua musica esta pronta para baixar:',
    linkUrl: `${SITE}/p/${order.id}`,
  });
  return sendSmsBatch([{ to, tipoEnvio: '2', msg, id: `ent-${order.id}`.slice(0, 50), form, linkUrl }]);
}

module.exports = { sendSmsBatch, sendRecoverySms, sendDeliverySms, firstNameSafe, formatPhone, stripAccents, buildMsg, SMS_ENABLED };
