...
// Pool de variantes ATIVAS no A/B. Default B,C — o assunto A ("está quase
// pronta") converteu bem menos e foi aposentado (decisão do dono 01/jul). Dá pra
// ajustar por env RECOVERY_EMAIL_VARIANTS (csv, ex: "B,C" ou "A,B,C") sem mexer
// no código — útil pra reativar o A ou testar um novo assunto depois.
const VARIANT_POOL = (() => {
  const raw = (process.env.RECOVERY_EMAIL_VARIANTS || 'B,C')
    .split(',').map(s => s.trim().toUpperCase()).filter(k => VARIANTS[k])
