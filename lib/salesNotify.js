...
// Pushcut DEDICADO da variante de preço p47 (teste A/B) — dispara um push
// distinto sempre que uma venda p47 (R$47) confirma, pra acompanhar o teste ao
// vivo. URL simples (nao JSON) pra nao arriscar o PUSHCUT_URL_BY_AMOUNT.
const PUSHCUT_P47 = String(process.env.PUSHCUT_URL_P47 || '').trim();
...
async function _firePushcut(amount) {
  // Tenta lookup em multiplas variantes pra tolerar formato do JSON env:
  // "19.90" (ponto) | "19,90" (virgula BR) | "1990" (centavos) | 19.9 (number-ish)
  const candidates = [
    Number(amount).toFixed(2),                  // "19.90"
    Number(amount).toFixed(2).replace('.', ','),// "19,90"
    String(amount),                              // "19.9" ou "19.90"
    String(Math.round(Number(amount) * 100)),    // "1990"
  ];
  let url = null;
  for (const k of candidates) {
    if (PUSHCUT_BY_AMOUNT[k]) { url = PUSHCUT_BY_AMOUNT[k]; break; }
  }
  if (!url) { console.log(`[salesNotify] sem Pushcut configurado pro valor ${amount} (tentei: ${candidates.join(', ')}), pulando`); return; }
...
async function notifySale(orderId) {
  try {
    const rows = await supaFetch('GET', `orders?id=eq.${orderId}&select=id,customer_name,customer_email,phone,honoree_name,relationship,plan,price_variant,payment_amount,paid_at,fbp_pixel_id,utm_source,utm_campaign,utm_medium,utm_term,utm_content,src`);
    ...
    Promise.all([
      ...
      _firePushcut(amount).catch(e => console.error('[salesNotify] pushcut erro:', e.message)),
      // Push dedicado do teste de preco: venda da variante p47.
      (o.price_variant === 'p47' && PUSHCUT_P47
        ? axios.post(PUSHCUT_P47, {}, { timeout: 10000 })
            .then(() => console.log(`[salesNotify] Pushcut p47 disparado (order ${o.id})`))
            .catch(e => console.error('[salesNotify] pushcut p47 erro:', e.message))
        : Promise.resolve()),
    ]).catch(() => {});
