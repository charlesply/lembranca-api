...
async function findOrphans() {
  ...
  // Pega qualquer order que tem ALGUMA pista pra resgatar áudio:
  // suno_task_id OU suno_clip_ids preenchidos, sem original_audio_url.
  // (Bug antigo: filtro só pegava suno_task_id — perdia orders que só
  // tinham clip_ids salvos via webhook.)
  const url = `${SUPABASE_URL}/orders?status=in.(preview_sent,paid,delivered)&original_audio_url=is.null&or=(suno_task_id.not.is.null,suno_clip_ids.not.is.null)&created_at=gte.${since}&order=created_at.desc&limit=30&select=id,status,paid_at,suno_task_id,suno_clip_ids,honoree_name`;
  try {
