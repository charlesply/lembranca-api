...
async function runRecoveryOnce() {
  ...
  try {
    ...
    // ⚠️ preview_sent_at é NULL em prod (pipeline nunca preenche) → usamos created_at
    // pra a janela + preview_audio_url=not.null pra garantir que a prévia está pronta.
    const q = `orders?status=eq.preview_sent&paid_at=is.null&recovery_email_sent=eq.false&email_opt_out=eq.false`
      + `&customer_email=not.is.null&preview_audio_url=not.is.null&created_at=gte.${lo}&created_at=lte.${hi}`
      + `&select=${cols}&order=created_at.asc&limit=${PER_RUN}`
