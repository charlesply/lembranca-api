...
// Escolhe a URL de áudio TOCÁVEL do track.
// 🚨 28/ago/2026: a Suno/revendedor passou a exigir URL ASSINADA (Key-Pair-Id)
// em `cdn1.suno.ai` e `musicfile.removeai.ai` — o link cru volta "MissingKey" e
// o player fica 0:00. Só `tempfile.aiquickdraw.com` continua servindo direto.
// Então preferimos QUALQUER campo do track que aponte pro aiquickdraw; senão o
// primeiro que NÃO seja host assinado; senão o que vier. Ver
// reference_suno_cdn_missingkey.
function _pickPlayableUrl(t) {
  const cands = [
    t.audioUrl, t.audio_url,
    t.sourceAudioUrl, t.source_audio_url,
    t.streamAudioUrl, t.stream_audio_url,
  ].map((u) => String(u || '').trim()).filter(Boolean);
  const SIGNED = /cdn1\.suno\.ai|musicfile\.removeai\.ai/i;
  return cands.find((u) => /aiquickdraw/i.test(u))
      || cands.find((u) => !SIGNED.test(u))
      || cands[0] || '';
}
...
// URL do clipe no CDN do Suno (cdn1.suno.ai). ⚠️ 28/ago/2026: a Suno passou a
// exigir URL assinada (Key-Pair-Id) aqui — o link cru volta "MissingKey" e não
// toca. Portanto NÃO é mais o link "permanente" que guardávamos; agora serve só
// de FALLBACK quando a API não devolve nenhuma URL tocável (aiquickdraw). Ver
// _pickPlayableUrl e reference_suno_cdn_missingkey.
