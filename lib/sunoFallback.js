...
// Hosts que passaram a exigir URL assinada (Key-Pair-Id) em 28/ago/2026 → o link
// cru volta "MissingKey" e não toca. Ver reference_suno_cdn_missingkey.
const SIGNED_HOST = /cdn1\.suno\.ai|musicfile\.removeai\.ai/i;
// Escolhe a URL TOCÁVEL do track. Aceita tanto o formato do callback da Suno
// (audioUrl/sourceAudioUrl/streamAudioUrl) quanto o normalizado do getTaskStatus
// (audio_url). Prefere aiquickdraw; senão qualquer host não-assinado; nunca
// fabrica cdn1 (que está quebrado). Ver lib/sunoApi._pickPlayableUrl.
function _pickPlayable(t) {
  const cands = [t.audio_url, t.audioUrl, t.sourceAudioUrl, t.source_audio_url,
                 t.streamAudioUrl, t.stream_audio_url]
    .map((u) => String(u || '').trim()).filter(Boolean);
  return cands.find((u) => /aiquickdraw/i.test(u))
      || cands.find((u) => !SIGNED_HOST.test(u))
      || cands[0] || '';
}

// Extrai URLs e clipIds de uma resposta do record-info/callback da Suno API.
// 🚨 28/ago/2026: NÃO fabricamos mais cdn1.suno.ai/{id}.mp3 — esse host passou a
// exigir URL assinada (MissingKey) e está quebrado. Guardamos a URL tocável que
// a Suno devolve (tempfile.aiquickdraw). ⚠️ tempfile expira ~2 semanas.
function tracksToUrls(tracks) {
  if (!Array.isArray(tracks)) return { urls: [], ids: [] };
  const urls = [], ids = [];
  for (const t of tracks) {
    const id = t.id || t.clipId;
    const u = _pickPlayable(t);
    if (id) ids.push(id);
    if (u) urls.push(u);
  }
  return { urls, ids };
