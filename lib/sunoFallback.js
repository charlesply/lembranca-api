...
function tracksToUrls(tracks) {
  ...
  for (const t of tracks) {
    ...
    // 🚨 SEMPRE preferir o cdn1 PERMANENTE (derivado do id). O audioUrl do
    // record-info aponta pra tempfile.aiquickdraw.com e EXPIRA em ~2 semanas —
    // era a fonte das músicas "sumindo". Só cai pro audioUrl se faltar o id.
    const u = (id ? `${CDN_BASE}/${id}.mp3` : null)
              || t.audioUrl || t.audio_url || t.sourceAudioUrl || null;
    if (id) ids.push(id);
    if (u) urls.push(u);
  }
  return { urls, ids };
