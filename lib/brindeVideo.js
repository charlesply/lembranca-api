...
async function _pollJob(jobId) {
  for (let i = 0; i < POLL_ITERATIONS; i++) {
    try {
      ...
      // 404 = job sumiu (video-api reiniciou/redeployou e perdeu a fila em memória).
      // Trata como falho → o chamador limpa o job_id e RE-SUBMETE no próximo tick.
      // Sem isso, o pedido fica preso pra sempre com um job_id morto (causa do backlog).
      if (e.response && e.response.status === 404) return { done: true, url: null, failed: true, orphaned: true };
      /* outros erros (rede/timeout): keep polling */
    }
...
async function _finalizeVideo(orderId, externalUrl, extraPatch) {
  ...
  await supaFetch('PATCH', `orders?id=eq.${orderId}`, {
    ...
    video_upsell_status: 'brinde_delivered', // limpa o 'brinde_pending' — extraPatch (upsell) sobrescreve
    ...(extraPatch || {}),
  });
  // 🎬 E-mail "vídeo pronto" pra vídeo ATRASADO: cliente pagou há tempo (>60min)
  // e o vídeo só ficou pronto agora (ex: os que falharam no AssemblyAI). NÃO
  // dispara no fluxo fresco (vídeo sai minutos após o pagamento). 1x por pedido.
  try {
    const rows = await supaFetch('GET', `orders?id=eq.${orderId}&select=id,honoree_name,customer_name,customer_email,paid_at,video_email_sent`);
    const o = Array.isArray(rows) && rows[0];
    if (o && o.customer_email && o.paid_at && !o.video_email_sent) {
      const ageMin = (Date.now() - new Date(o.paid_at).getTime()) / 60000;
      if (ageMin > 60) {
        const { sendVideoReadyEmail } = require('./emailDelivery');
        const r = await sendVideoReadyEmail(o);
        if (r && r.ok) {
          await supaFetch('PATCH', `orders?id=eq.${orderId}`, { video_email_sent: true, video_email_sent_at: new Date().toISOString() });
          console.log(`[BrindeVideo] 📧 e-mail 'vídeo pronto' enviado p/ ${orderId}`);
        }
      }
    }
  } catch (e) { console.error('[BrindeVideo] video-ready email err', orderId, e.message); }
}
...
// Single GET no job (NAO bloqueia). done→{done,url} · 404→{orphaned} · erro→{failed} · senao {pending}.
async function _pollJobOnce(jobId) {
  try {
    const j = await axios.get(`${VIDEO_API}/api/jobs/${jobId}`, { headers: { 'X-API-Key': VIDEO_KEY }, timeout: 12000 });
    const st = j.data && j.data.status;
    if (st === 'done') return { done: true, url: j.data.public_url || j.data.video_url };
    if (st === 'error' || st === 'failed') return { failed: true };
    return { pending: true };
  } catch (e) {
    if (e.response && e.response.status === 404) return { orphaned: true }; // job sumiu (restart do video-api)
    return { pending: true }; // erro de rede: tenta de novo no proximo tick
  }
}

// RECONCILE robusto: single-poll de TODOS os jobs em voo e salva TODA conclusao,
// independente da ordem em que o video-api completa. Era a causa do backlog preso:
// o video-api completa fora de ordem e o poll antigo (limitado aos 20 mais antigos)
// perdia as conclusoes → vídeo era feito e jogado fora. Orfaos (404) re-submetem.
async function _reconcileAllPending() {
  const rows = await supaFetch('GET', `orders?paid_at=not.is.null&video_brinde_url=is.null&video_brinde_job_id=not.is.null&select=id,video_brinde_job_id&limit=500`);
  const list = Array.isArray(rows) ? rows : [];
  let saved = 0, resub = 0;
  for (let i = 0; i < list.length; i += 20) {
    await Promise.all(list.slice(i, i + 20).map(async o => {
      const r = await _pollJobOnce(o.video_brinde_job_id);
      if (r.done && r.url) { await _finalizeVideo(o.id, r.url).catch(() => {}); saved++; }
      else if (r.orphaned || r.failed) { await _clearJobMarkers(o.id).catch(() => {}); resub++; }
    }));
  }
  if (saved || resub) console.log(`[BrindeVideo] reconcile: salvos=${saved} re-submeter=${resub} (${list.length} em voo)`);
  return { saved, resub, inflight: list.length };
}
...
// Cria o job do video pra UM pedido e PERSISTE o job_id — SEM pollar (retorna
// rapido). Usado pelo self-edit (regenerateEditedSong): a regen chama isso pra
// nao depender do cron `newOnes` (que ordena por paid_at.desc e faminava pedidos
// antigos). Depois de persistido, o cron `pendingJobs` (ordenado por
// started_at.asc, sem starvation) retoma o poll e finaliza.
async function createBrindeJob(orderId, imageUrl) {
  if (!orderId) return null;
  try {
    const rows = await supaFetch('GET', `orders?id=eq.${orderId}&select=id,honoree_name,relationship,genre,story,original_audio_url,preview_audio_url,video_brinde_url,final_lyrics`);
    const o = Array.isArray(rows) && rows[0];
    if (!o || o.video_brinde_url) return null;
    const audio = o.original_audio_url || o.preview_audio_url;
    if (!audio) return null;
    let cover = imageUrl;
    if (!cover) cover = await _pexelsImage(_categoryQueries(o));
    const jobId = await _submitJob(audio, 'Para ' + (o.honoree_name || ''), cover, o.final_lyrics);
    if (!jobId) { console.error(`[BrindeVideo] createBrindeJob: _submitJob null p/ ${orderId}`); return null; }
    await supaFetch('PATCH', `orders?id=eq.${orderId}`, {
      video_brinde_job_id: jobId,
      video_brinde_started_at: new Date().toISOString(),
    });
    console.log(`[BrindeVideo] createBrindeJob ${jobId} p/ ${orderId} — cron pendingJobs finaliza`);
    return jobId;
  } catch (e) { console.error('[BrindeVideo] createBrindeJob erro:', e.message); return null; }
}
...
async function runBrindeGenOnce() {
  try {
    ...
    // Gate de pagamento = `paid_at NOT NULL` (fonte de verdade), NAO `status=paid`.
    // MUDANCA 29/jun: antes filtrava status=in.(paid,delivered) — mas pedidos pagos
    // ficam com status='preview_sent' quando o fluxo de delivery nao avanca o status,
    // mesmo com paid_at + abacate=PAID. Isso fazia o cron PULAR ~29 completas pagas
    // → cliente pagava R$29,90 e nunca recebia o video. paid_at cobre paid E delivered.
    //
    // Passo 1: RECONCILE — single-poll TODOS os jobs em voo, salva toda conclusao
    // (robusto a ordem em que o video-api completa) e re-submete orfaos (404).
    // Substitui o block-poll limitado aos 20 mais antigos que perdia conclusoes.
    await _reconcileAllPending();
    // Passo 2: cria job p/ novas (mais recentes) E backlog (mais antigas) —
    // dreno os dois ao mesmo tempo, pra não faminar nem quem comprou agora nem quem esperou.
    const _base = `orders?paid_at=not.is.null&${planFilter}&video_brinde_url=is.null&video_brinde_job_id=is.null&original_audio_url=not.is.null&select=id`;
    const _newR = await supaFetch('GET', `${_base}&order=paid_at.desc&limit=6`);
    const _oldR = await supaFetch('GET', `${_base}&order=paid_at.asc&limit=6`);
    const _seen = new Set();
    const newOnes = [...(Array.isArray(_newR) ? _newR : []), ...(Array.isArray(_oldR) ? _oldR : [])].filter(o => !_seen.has(o.id) && _seen.add(o.id));
...
module.exports = { generateBrindeForOrder, generatePersonalizedForOrder, createBrindeJob, runBrindeGenOnce, startBrindeGenCron };
