...
// Limite 4 em paralelo (cada um pode levar 20min em poll, mas com persistencia
// nao gera dup). Premium legado: max 2.
async function runBrindeGenOnce() {
  try {
    const rows = await supaFetch('GET', `orders?status=in.(preview_sent,paid,delivered)&video_brinde_url=is.null&original_audio_url=not.is.null&select=id&order=created_at.desc&limit=4`);
    await Promise.all((Array.isArray(rows) ? rows : []).map(o =>
      generateBrindeForOrder(o.id).catch(e => console.error('[BrindeVideo] order err:', o.id, e.message))
    ));
