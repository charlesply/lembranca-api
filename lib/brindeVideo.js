...
async function runBrindeGenOnce() {
  try {
    ...
    const pendingJobs = await supaFetch('GET', `orders?paid_at=not.is.null&${planFilter}&video_brinde_url=is.null&video_brinde_job_id=not.is.null&select=id&order=video_brinde_started_at.asc&limit=20`);
