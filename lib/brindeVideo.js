...
async function runBrindeGenOnce() {
  try {
    ...
    const pendingJobs = await supaFetch('GET', `orders?status=in.(preview_sent,paid,delivered)&video_brinde_url=is.null&video_brinde_job_id=not.is.null&select=id&order=video_brinde_started_at.asc&limit=8`);
    ...
    // Passo 2: ordens novas precisando criar job.
    const newOnes = await supaFetch('GET', `orders?status=in.(preview_sent,paid,delivered)&video_brinde_url=is.null&video_brinde_job_id=is.null&original_audio_url=not.is.null&select=id&order=created_at.desc&limit=4`);
