...
async function runBrindeGenOnce() {
  try {
    ...
    const pendingJobs = await supaFetch('GET', `orders?paid_at=not.is.null&${planFilter}&video_brinde_url=is.null&video_brinde_job_id=not.is.null&select=id&order=video_brinde_started_at.asc&limit=8`);
    ...
    const newOnes = await supaFetch('GET', `orders?paid_at=not.is.null&${planFilter}&video_brinde_url=is.null&video_brinde_job_id=is.null&original_audio_url=not.is.null&select=id&order=paid_at.desc&limit=4`);
