...
// Reply-to de suporte: quando o cliente responde o e-mail de entrega/prévia,
// cai no suporte (e não na caixa da Bia).
const SUPPORT_REPLY_TO = 'suporte@lembrancacantada.com'
...
async function sendDeliveryEmail(order) {
  ...
  // ── ENVIO DUPLO (A/B checkout, variante B) ──────────────────────────────
  // Se o cliente digitou um e-mail DIFERENTE no checkout, a música vai pros
  // DOIS (o do pedido + o do checkout). Mitiga typo: se ele errou num, recebe
  // no outro. checkout_email é buscado se não veio no objeto do order.
  let checkoutEmail = order.checkout_email
  if (checkoutEmail === undefined) {
    try {
      const rr = await supaFetch('GET', `orders?id=eq.${order.id}&select=checkout_email`)
      checkoutEmail = (Array.isArray(rr) && rr[0]) ? rr[0].checkout_email : null
    } catch (_) { checkoutEmail = null }
  }
  const _validEmail = (e) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(e || ''))
  const recipients = [order.customer_email]
  if (checkoutEmail && _validEmail(checkoutEmail)
    && String(checkoutEmail).toLowerCase() !== String(order.customer_email).toLowerCase()) {
  ...
  try {
    ...
    console.log('[emailDelivery] ✅ enviado order', order.id, 'to', recipients.join(', '), 'id', emailId)
...
// ─────────────────────────────────────────────────────────────────────────
// E-mail de DESCULPA — pro lead cuja prévia falhou/atrasou (ex: queda SUNOAPI).
// Prévia agora pronta → link /finalizar (ouvir + liberar). Idempotência: usa a
// coluna preview_recovery_email_sent (não colide com recovery/delivery).
async function sendApologyPreviewEmail(order) {
  if (!RESEND_API_KEY) return { ok: false, skipped: 'no_key' }
  if (!order || !order.id || !order.customer_email) return { ok: false, skipped: 'no_email' }
  const honoree = order.honoree_name || 'você'
  const first = order.customer_name ? esc(order.customer_name.split(/\s+/)[0]) : ''
  const greeting = first ? `Olá, ${first}!` : 'Olá!'
  const url = `${APP_URL}/finalizar/${order.id}`
  const html = `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml" lang="pt-BR"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>Sua prévia está pronta</title></head>
<body style="margin:0; padding:0; background:#fef9f5; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; color:#2b1d14;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background:#fef9f5; padding:32px 16px;"><tr><td align="center">
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="560" style="max-width:560px; background:#fff; border-radius:20px; border:1px solid #f3e5d8; box-shadow:0 12px 40px rgba(204,120,92,0.08);">
    <tr><td style="padding:40px 32px 8px; text-align:center;"><div style="font-size:48px; line-height:1;">🎵</div></td></tr>
    <tr><td style="padding:8px 32px 12px; text-align:center;"><h1 style="margin:0; font-size:25px; font-weight:700; line-height:1.25; color:#2b1d14;">Sua prévia para <span style="color:#CC785C;">${esc(honoree)}</span> ficou pronta!</h1></td></tr>
    <tr><td style="padding:0 32px 22px; text-align:center;"><p style="margin:0; font-size:16px; color:#7a6354; line-height:1.55;">${greeting} 💛<br/>Hoje tivemos um <strong>volume altíssimo de pedidos</strong> e a sua prévia acabou demorando mais que o normal — <strong>desculpa de verdade!</strong> Já está tudo certo: ela ficou linda e está pronta pra você ouvir.</p></td></tr>
    <tr><td style="padding:0 32px 30px; text-align:center;"><a href="${esc(url)}" style="display:inline-block; background:#CC785C; color:#ffffff !important; text-decoration:none; padding:16px 36px; border-radius:12px; font-weight:700; font-size:16px; box-shadow:0 6px 16px rgba(204,120,92,0.25);">🎧 Ouvir minha prévia</a></td></tr>
    <tr><td style="padding:22px 32px 32px; text-align:center; border-top:1px solid #f3e5d8;"><p style="margin:0; font-size:13px; color:#a09080; line-height:1.5;">Obrigada pela paciência 💛<br/><strong>Lembrança Cantada</strong></p></td></tr>
  </table>
</td></tr></table></body></html>`
  const text = [greeting + ' 💛', '',
    `Hoje tivemos um volume altíssimo de pedidos e a sua prévia para ${honoree} demorou mais que o normal — desculpa de verdade!`,
    'Já está pronta pra você ouvir:', url, '', 'Obrigada pela paciência,', 'Bia da Lembrança Cantada 💛'].join('\n')
  try {
    const r = await axios.post('https://api.resend.com/emails', {
      from: `${FROM_NAME} <${FROM_EMAIL}>`,
      to: [order.customer_email], reply_to: REPLY_TO,
      subject: `🎵 Sua prévia para ${honoree} ficou pronta — desculpe a demora!`,
      html, text, tags: [{ name: 'kind', value: 'apology-preview' }],
    }, { headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 15000 })
    console.log('[emailDelivery/apology] ✅ order', order.id, 'to', order.customer_email, 'id', r.data?.id)
    return { ok: true, id: r.data?.id }
  } catch (e) {
    console.error('[emailDelivery/apology] ❌ order', order.id, ':', JSON.stringify(e.response?.data || e.message).slice(0, 300))
    return { ok: false, error: e.response?.data || e.message }
  }
}

// ─────────────────────────────────────────────────────────────────────────
// E-mail "🎧 sua NOVA PRÉVIA está pronta" — pro cliente NÃO-PAGO que ajustou a
// própria prévia (self-edit na PaymentPage/painel). Diferente do edit-ready
// (que é pós-pago e leva pro /p/): aqui a CTA leva pro /finalizar pra PAGAR a
// nova prévia. Idempotência fica por conta do step do Inngest (1x por regen).
function renderPreviewReadyHtml({ honoree, customerName, payUrl }) {
  const greeting = customerName ? `Olá, ${esc(customerName.split(/\s+/)[0])}!` : 'Olá!'
  return `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml" lang="pt-BR">
<head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>Sua nova prévia está pronta!</title></head>
<body style="margin:0; padding:0; background:#fef9f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color:#2b1d14;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background:#fef9f5; padding:32px 16px;">
  <tr><td align="center">
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="560" style="max-width:560px; background:#ffffff; border-radius:20px; border:1px solid #f3e5d8; box-shadow:0 12px 40px rgba(204,120,92,0.08);">
      <tr><td style="padding:40px 32px 8px; text-align:center;"><div style="font-size:48px; line-height:1;">✨🎧</div></td></tr>
      <tr><td style="padding:8px 32px 16px; text-align:center;">
        <h1 style="margin:0; font-size:26px; font-weight:700; line-height:1.25; color:#2b1d14;">Sua nova prévia para <span style="color:#CC785C;">${esc(honoree)}</span> ficou pronta!</h1>
      </td></tr>
      <tr><td style="padding:0 32px 24px; text-align:center;">
        <p style="margin:0; font-size:16px; color:#7a6354; line-height:1.5;">${greeting} 💛<br/>Criamos sua prévia nova com o ajuste que você pediu. Ouça e, se gostar, é só finalizar pra liberar a <strong>versão completa</strong> em alta qualidade e sem marca d'água.</p>
      </td></tr>
      <tr><td style="padding:0 32px 32px; text-align:center;">
        <a href="${esc(payUrl)}" style="display:inline-block; background:#CC785C; color:#ffffff !important; text-decoration:none; padding:16px 36px; border-radius:12px; font-weight:700; font-size:16px; box-shadow:0 6px 16px rgba(204,120,92,0.25);">🎧 Ouvir e finalizar</a>
      </td></tr>
      <tr><td style="padding:24px 32px 32px; text-align:center; border-top:1px solid #f3e5d8;">
        <p style="margin:0; font-size:13px; color:#a09080; line-height:1.5;"><strong>Lembrança Cantada</strong> · Feito com carinho 💛</p>
      </td></tr>
...
async function sendPreviewReadyEmail(order) {
  if (!RESEND_API_KEY) { console.warn('[emailDelivery/preview] RESEND_API_KEY ausente'); return { ok: false, skipped: 'no_key' } }
  if (!order || !order.id) return { ok: false, skipped: 'no_order' }
  if (!order.customer_email) return { ok: false, skipped: 'no_email' }
  const payUrl = `${APP_URL}/finalizar/${order.id}?src=email_previa_nova`
  const honoree = order.honoree_name || 'você'
  const customerName = order.customer_name || ''
  const text = [
    customerName ? `Olá, ${customerName.split(/\s+/)[0]}!` : 'Olá!', '',
    `Sua nova prévia para ${honoree} ficou pronta, com o ajuste que você pediu!`,
    `Ouça e, se gostar, finalize pra liberar a versão completa em alta qualidade e sem marca d'água.`,
    '', `🎧 Ouvir e finalizar:`, payUrl, '',
    `Um abraço carinhoso,`, `Bia da Lembrança Cantada 💛`,
  ].join('\n')
  try {
    const r = await axios.post('https://api.resend.com/emails', {
      ...
      to: [order.customer_email],
      reply_to: SUPPORT_REPLY_TO,
      subject: `✨ Sua nova prévia para ${honoree} ficou pronta!`,
      html: renderPreviewReadyHtml({ honoree, customerName, payUrl }),
      text,
      tags: [{ name: 'kind', value: 'preview-ready' }],
    }, { headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 15000 })
    console.log('[emailDelivery/preview] ✅ enviado order', order.id, 'to', order.customer_email, 'id', r.data?.id)
    return { ok: true, id: r.data?.id }
  } catch (e) {
    console.error('[emailDelivery/preview] ❌ erro order', order.id, ':', JSON.stringify(e.response?.data || e.message).slice(0, 300))
    return { ok: false, error: e.response?.data || e.message }
  }
}

// ─────────────────────────────────────────────────────────────────────────
// ALERTA INTERNO (pro DONO) — uma prévia FALHOU e não foi gerada. Manda pro
// Charles identificando o cliente afetado, pra ele agir (regenerar/reembolsar).
// NÃO vai pro cliente. Remetente transacional; destino fixo (ALERT_EMAIL).
const ALERT_EMAIL = process.env.ALERT_EMAIL || 'charles.cmh7@gmail.com'
async function sendPreviewErrorAlert(order, errorMsg) {
  // Throttle 20min: num apagão total (centenas de falhas) sem isso, cada falha
  // mandava 1 e-mail → enxurrada na caixa do dono. 1 alerta a cada 20min basta.
  if (Date.now() - (sendPreviewErrorAlert._last || 0) < 1200000) { console.log('[previewAlert] throttle 20min — pulando'); return { ok: false, skipped: 'throttle' } }
  sendPreviewErrorAlert._last = Date.now()
  if (!RESEND_API_KEY) { console.warn('[previewAlert] RESEND_API_KEY ausente'); return { ok: false, skipped: 'no_key' } }
  if (!order || !order.id) return { ok: false, skipped: 'no_order' }
  const linkPedido = `${APP_URL}/finalizar/${order.id}`
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#2b1d14;max-width:560px">
    <h2 style="color:#b04a30;margin:0 0 12px">⚠️ Falha ao gerar prévia</h2>
    <p style="margin:0 0 14px;color:#6b5a4d">Uma prévia não foi gerada e o cliente <b>não conseguiu pagar</b> (o pagamento fica bloqueado sem prévia). Verifique/regenere:</p>
    <table style="border-collapse:collapse;font-size:14px">
      <tr><td style="padding:4px 12px 4px 0;color:#8a7969">Homenageado</td><td><b>${esc(order.honoree_name || '—')}</b></td></tr>
      <tr><td style="padding:4px 12px 4px 0;color:#8a7969">Cliente</td><td>${esc(order.customer_name || '—')}</td></tr>
      <tr><td style="padding:4px 12px 4px 0;color:#8a7969">E-mail</td><td>${esc(order.customer_email || '—')}</td></tr>
      <tr><td style="padding:4px 12px 4px 0;color:#8a7969">Telefone</td><td>${esc(order.phone || '—')}</td></tr>
      <tr><td style="padding:4px 12px 4px 0;color:#8a7969">Pedido</td><td>${esc(order.id)}</td></tr>
      <tr><td style="padding:4px 12px 4px 0;color:#8a7969">Erro</td><td>${esc(String(errorMsg || '').slice(0, 300))}</td></tr>
    </table>
    <p style="margin:16px 0 0"><a href="${esc(linkPedido)}" style="color:#CC785C">Abrir pedido</a></p>
  </div>`
  const text = [
    'FALHA AO GERAR PRÉVIA — cliente não conseguiu pagar (pagamento bloqueado sem prévia).',
    `Homenageado: ${order.honoree_name || '—'}`,
    `Cliente: ${order.customer_name || '—'} | ${order.customer_email || '—'} | ${order.phone || '—'}`,
    `Pedido: ${order.id}`,
    `Erro: ${String(errorMsg || '').slice(0, 300)}`,
    `Link: ${linkPedido}`,
  ].join('\n')
  try {
    const r = await axios.post('https://api.resend.com/emails', {
      ...
      to: [ALERT_EMAIL],
      subject: `⚠️ Falha na prévia — ${order.honoree_name || 'cliente'} (${order.customer_name || order.phone || order.id.slice(0, 8)})`,
      html, text,
      tags: [{ name: 'kind', value: 'preview-error-alert' }],
    }, { headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 15000 })
    console.log('[previewAlert] ✅ alerta enviado p/', ALERT_EMAIL, 'order', order.id, 'id', r.data?.id)
    return { ok: true, id: r.data?.id }
  } catch (e) {
    console.error('[previewAlert] ❌ erro:', JSON.stringify(e.response?.data || e.message).slice(0, 300))
    return { ok: false, error: e.response?.data || e.message }
  }
}

// Alerta de QUEDA da sunoapi — disparado quando N falhas de geração acontecem
// numa janela curta (ver detector no webhookRoutes). Sinal de que a sunoapi.org
// parou de gerar (ex: SENSITIVE_WORD_ERROR em tudo, 5xx). Vai pro ALERT_EMAIL.
async function sendSunoOutageEmail({ count, windowSec }) {
  if (!RESEND_API_KEY) return { ok: false, skipped: 'no_key' }
  const mins = Math.round((windowSec / 60) * 10) / 10
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#2b1d14;max-width:560px">
    <h2 style="color:#b04a30;margin:0 0 12px">🔴 SUNOAPI pode ter caído</h2>
    <p style="margin:0 0 12px;color:#6b5a4d"><b>${count} falhas de geração em menos de ${mins} min</b> — forte sinal de que a sunoapi.org parou de gerar (ex.: SENSITIVE_WORD_ERROR em tudo, ou 5xx).</p>
    <p style="margin:0 0 12px;color:#6b5a4d">Enquanto isso <b>ninguém consegue pagar</b> (sem prévia = sem pagamento). Confira a sunoapi.org / considere trocar o provider de geração.</p>
    <p style="margin:16px 0 0;color:#8a7969;font-size:12px">Dedup de 20min. — monitor lembrança-api</p>
  </div>`
  try {
    const r = await axios.post('https://api.resend.com/emails', {
      from: `Alertas Lembrança Cantada <${FROM_EMAIL}>`,
      to: [ALERT_EMAIL],
      subject: `🔴 SUNOAPI caiu? ${count} falhas de geração em <${mins}min`,
      html,
      text: `SUNOAPI pode ter caido: ${count} falhas de geracao em <${windowSec}s. Ninguem consegue pagar (sem previa). Verifique a sunoapi.org.`,
      tags: [{ name: 'kind', value: 'suno-outage' }],
    }, { headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, timeout: 12000 })
    console.log('[sunoOutage] ✅ alerta de queda enviado p/', ALERT_EMAIL, 'id', r.data?.id)
    return { ok: true, id: r.data?.id }
  } catch (e) {
    console.error('[sunoOutage] ❌ erro:', JSON.stringify(e.response?.data || e.message).slice(0, 300))
    return { ok: false, error: e.response?.data || e.message }
  }
}

module.exports = { sendDeliveryEmail, sendEditReadyEmail, sendVideoReadyEmail, sendPreviewReadyEmail, sendApologyPreviewEmail, sendPreviewErrorAlert, sendSunoOutageEmail, renderHtml, renderText }
