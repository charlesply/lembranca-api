...
async function _gerarVideo(audioUrl, imageUrl, title, lyrics) {
  const p = { audio_url: audioUrl, title: title, image_url: imageUrl };
  // Letra OFICIAL (final_lyrics) = fonte de verdade das palavras; AssemblyAI só dá
  // o TEMPO de cada palavra (forced alignment). Ver app.py do video-api.
  if (lyrics && String(lyrics).trim().length > 10) p.lyrics = String(lyrics).trim();
...
async function runUpsellVideoOnce(reason = 'cron') {
  ...
  try {
    const rows = await supaFetch('GET',
      `orders?video_upsell_status=eq.photo_received&original_audio_url=not.is.null&select=id,phone,honoree_name,original_audio_url,customer_photo_msg_id,customer_photo_url,final_lyrics&limit=5`);
    ...
    for (const o of pend) {
      ...
      try {
        ...
        const videoUrl = await _gerarVideo(o.original_audio_url, imgUrl, 'Para ' + (o.honoree_name || ''), o.final_lyrics);
