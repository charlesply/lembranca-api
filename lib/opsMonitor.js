...
async function checkAbacate() {
  ...
  const allFailed = fails === attempts;
  await _evaluate('abacate', allFailed, 'AbacatePay não está gerando PIX', [
    [`Tentativas (últ. ${FAST_MIN} min)`, String(attempts)],
    ['Falhas', String(fails)],
    ['Ação', 'Checar AbacatePay — considerar virar PIX_PROVIDER=woovi'],
  ]);
  return { attempts, fails, allFailed };
}
...
async function checkNoSales() {
  ...
  let rows;
  try { rows = await supaFetch('GET', 'orders?paid_at=not.is.null&order=paid_at.desc&limit=1&select=paid_at'); }
  catch (e) { return { error: e.message }; }
  const last = Array.isArray(rows) && rows[0] && rows[0].paid_at ? new Date(rows[0].paid_at).getTime() : null;
  const gapMin = last ? Math.round((Date.now() - last) / 60000) : 9999;
  await _evaluate('nosales', gapMin >= NO_SALES_MIN, `Sem vendas há ${gapMin} min`, [
    ['Última venda', last ? `${_brtStamp(last)} BRT` : '— (nenhuma)'],
    ['Gap', `${gapMin} min`],
...
async function checkInngest() {
  ...
  let leads, prev;
  try {
    leads = await supaFetch('GET', `orders?created_at=gte.${since}&select=id&limit=${INNGEST_MIN_LEADS}`);
    ...
    prev = await supaFetch('GET', `orders?preview_audio_url=not.is.null&updated_at=gte.${since}&select=id&limit=1`);
  } catch (e) { return { error: e.message }; }
  const nLeads = Array.isArray(leads) ? leads.length : 0;
  const nPrev = Array.isArray(prev) ? prev.length : 0;
...
async function runFast() {
  try { const r = await checkAbacate(); console.log('[opsMonitor] fast(abacate)', JSON.stringify(r)); return r; }
  catch (e) { console.error('[opsMonitor] fast err', e.message); }
}

async function runSlow() {
  const out = {};
  for (const [k, fn] of [['disk', checkDisk], ['suno', checkSuno], ['supabase', checkSupabase], ['nosales', checkNoSales], ['inngest', checkInngest]]) {
...
function startCron() {
  ...
  console.log(`[opsMonitor] ✅ ON — AbacatePay a cada ${FAST_MIN}min; resto a cada ${SLOW_MIN}min; alertas → ${ALERT_EMAIL}`);
...
module.exports = {
  ...
  checkAbacate, checkDisk, checkSuno, checkSupabase, checkNoSales, checkInngest,
};
