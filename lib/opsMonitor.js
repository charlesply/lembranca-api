...
// Fallback ASAAS→Woovi: alerta quando a ASAAS começa a recusar (WAF/limite 2000/dia)
// e as cobranças caem no fallback Woovi. Passivo (payRoutes chama recordAsaasOutcome).
const ASAAS_MIN_ATTEMPTS   = num('OPS_ASAAS_MIN_ATTEMPTS', 8);   // volume mínimo p/ decidir
const ASAAS_FALLBACK_PCT   = num('OPS_ASAAS_FALLBACK_PCT', 30);  // % de fallback que dispara
...
// ── ring buffer ASAAS: ok=true → cobrança ASAAS criada; ok=false → caiu no fallback Woovi ──
const _asaas = []; // { t: ms, ok: bool }
function recordAsaasOutcome(ok) {
  _asaas.push({ t: Date.now(), ok: !!ok });
  const cut = Date.now() - 30 * 60 * 1000;
  while (_asaas.length && _asaas[0].t < cut) _asaas.shift();
}
...
async function checkAsaasFallback() {
  const cut = Date.now() - FAST_MIN * 60 * 1000;
  const recent = _asaas.filter(x => x.t >= cut);
  const attempts = recent.length;
  // Só decide com volume mínimo — evita falso positivo em janela fraca.
  if (attempts < ASAAS_MIN_ATTEMPTS) return { skipped: true, attempts };
  const fb = recent.filter(x => !x.ok).length;
  const pct = Math.round((fb / attempts) * 100);
  const bad = pct >= ASAAS_FALLBACK_PCT;
  await _evaluate('asaas_fallback', bad, `ASAAS recusando — ${pct}% caindo no fallback Woovi`, [
    [`Cobranças ASAAS (últ. ${FAST_MIN} min)`, String(attempts)],
    ['Caíram no fallback Woovi', `${fb} (${pct}%)`],
    ['Limiar de alerta', `${ASAAS_FALLBACK_PCT}%`],
    ['Provável causa', 'WAF/403 ou limite diário ASAAS (2000/dia)'],
    ['Ação', 'Woovi já cobre (ninguém sem pagar). Se piorar: PIX_PROVIDER=woovi + falar com ASAAS.'],
  ]);
  return { attempts, fb, pct, bad };
}
...
async function runFast() {
  const out = {};
  for (const [k, fn] of [['abacate', checkAbacate], ['asaas_fallback', checkAsaasFallback], ['payhealth', checkPaymentHealth], ['nosales', checkNoSales]]) {
...
module.exports = {
  startCron, recordAbacateOutcome, recordAsaasOutcome, runFast, runSlow,
  checkAbacate, checkAsaasFallback, checkPaymentHealth, checkDisk, checkSuno, checkSupabase, checkNoSales, checkInngest,
};
