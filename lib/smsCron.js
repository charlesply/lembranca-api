...
async function runRecovery(budget) {
  ...
  const msgs = [], ids = [];
  for (const o of rows) { const m = await buildRecoveryMsg(o); if (m) { msgs.push(m); ids.push(o.id); } }
...
async function runDelivery(budget) {
  ...
  const msgs = [], ids = [];
  for (const o of rows) { const m = await buildDeliveryMsg(o); if (m) { msgs.push(m); ids.push(o.id); } }
  if (!msgs.length) return { sent: 0 };
  const r = await sendSmsBatch(msgs);
...
async function runSmsOnce(mode) {
  ...
  try {
    // Só RECUPERAÇÃO (por tempo, 7min pós-prévia). A ENTREGA vai INSTANTÂNEA pelo
    // webhook de pagamento (lib/sms.deliverSmsOnPaid), igual o e-mail — sem cron.
    const recSent = await countSent('recovery');
    const rec = await runRecovery(RECOVERY_CAP() - recSent);
    const out = { ok: true, recovery: { jaEnviados: recSent, teto: RECOVERY_CAP(), enviadosAgora: rec.sent, atingiuTeto: !!rec.cap } };
    if (rec.sent) console.log('[smsCron]', JSON.stringify(out));
    return out;
